...
export const LabelInputField = (
  { input, required, width, meta: { touched, error }, ...rest }, //eslint-disable-line
) => (
...
export const Select = ({ input, required, options, meta: { touched, error }, ...rest }: OptionsFieldProps) => (
  <SelectComponent
    search
    value={input.value}
    required={required}
...
export const CheckboxField = ({ input, label, width, meta: { touched, error }, ...custom }: FieldProps) => (
  <Form.Field
...
export const Range = ({ input, min, max, required, meta: { touched, error }, ...rest }: RangeFieldProps) => (
  <input type="range" required={required} min={min} max={max} {...input} {...rest} />
);
...
export const Dropdown = ({ input, required, options, meta: { touched, error }, ...rest }: OptionsFieldProps) => (
  <DropdownComponent
    search
    value={input.value}
    required={required}
...
export const UploadField = ({
  ...
  return (
    <Form.Field error={touched && error} required={required} width={width} inline={inline}>
      {label && <label>{label}</label>}
      <InputComponent type="file" {...input} {...rest} />
      {touched && error ? (
        <Label basic color="red" pointing>
          {error}
        </Label>
...
export const Upload = ({ input, required, meta: { touched, error }, ...rest }: FieldProps) => {
  delete input.value;
  return <InputComponent required={required} type="file" {...input} {...rest} />;
};
